...
RUN if [ "$BETA" = "true" ]; then \
  echo "Installing Terratech BETA..."; \
  steamcmd +@sSteamCmdForcePlatformType windows \
  +force_install_dir "$TERRATECHDIR" \
  +login anonymous \
  +app_update 2533070 -beta ttw_unstable validate \
  +quit; \
  else \
  echo "Installing Terratech STABLE..."; \
  steamcmd +@sSteamCmdForcePlatformType windows \
  +force_install_dir "$TERRATECHDIR" \
  +login anonymous \
  +app_update 2533070 validate \
  +quit; \
  fi
